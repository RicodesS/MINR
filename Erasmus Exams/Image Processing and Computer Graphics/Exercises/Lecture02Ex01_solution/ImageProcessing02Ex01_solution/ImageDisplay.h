 ...
 * |                                                                 |
 * |   /// Update window with another image (e.g. for animations)    |
 * |   my_display.Display(another_ctensor_image, "Awesome Image");   |
 * |                                                                 |
 * |   /// Open a second window                                      |
 * |   my_display.Display(another_ctensor_image, "Another Window");  |
 * |                                                                 |
  ...
  public:
                ...
                )
    {
      ...
      if ( m_map.count( windowtitle ) > 0 ) {
        ...
        //m_map[windowtitle]->second->set_normalization(0);
        ...
        CImgDisplay* second = new CImgDisplay(tmp_cimg_ptr->width(),
                                              tmp_cimg_ptr->height(),
                                              windowtitle.c_str());
        /// Do not perform normalization
        second->set_normalization(0);
        second->display(*tmp_cimg_ptr);
        m_map[windowtitle] =  
